...
type ProductData = { sogamaxPrice: number; sogamaxCost: number; lastPurchaseCost: number; offers: MarketOffer[] };
...
const money = (value: number) => value > 0 ? value.toLocaleString("pt-BR", { style: "currency", currency: "BRL" }) : "—";
...
export default function Home() {
  ...
  const [purchaseSuggestions, setPurchaseSuggestions] = useState<Record<number, string>>({});
  ...
  return <main className="app-shell">
    <aside className="sidebar">
      ...
      <div className="sidebar-note"><span className="pulse" /><div><strong>Base integrada</strong><small>{formatCount(summaryData.sogamaxReferences)} descrições Sogamax</small></div></div>
    ...
    <section className="workspace">
      ...
      {view === "dashboard" && <div className="page dashboard-page">
        ...
        <section className="kpi-grid"><article><span>Linhas recebidas</span><strong>{formatCount(summaryData.marketLines)}</strong><small>100% preservadas</small></article><article><span>Descrições limpas</span><strong>{formatCount(summaryData.cleanDescriptions)}</strong><small>repetições agrupadas</small></article><article className="accent"><span>Referências Sogamax</span><strong>{formatCount(summaryData.sogamaxReferences)}</strong><small>da planilha hospitalar enviada</small></article><article><span>Validadas nesta sessão</span><strong>{approved.length}</strong><small>decisões reaplicadas às repetições</small></article></section>
        <section className="content-grid">
          <article className="panel status-panel"><div className="panel-head"><div><span className="eyebrow">QUALIDADE DA PADRONIZAÇÃO</span><h3>Confiança baseada na descrição</h3></div><button className="text-button" onClick={() => setView("validation")}>Ver tabela →</button></div><div className="status-bars">{[{ label: "Padronizado", value: summaryData.statusCounts.Padronizado, color: "#25a56a" }, { label: "Provável", value: summaryData.statusCounts.Provável, color: "#e7b53f" }, { label: "Revisão humana", value: summaryData.statusCounts.Revisar, color: "#e8833a" }].map((item) => <div className="bar-row" key={item.label}><div className="bar-label"><span>{item.label}</span><strong>{formatCount(item.value)}</strong></div><div className="bar-track"><div style={{ width: `${item.value / Math.max(...Object.values(summaryData.statusCounts)) * 100}%`, background: item.color }} /></div></div>)}</div></article>
          <article className="panel coverage-panel"><span className="eyebrow">GANHO OPERACIONAL</span><div className="coverage-number">{formatCount(summaryData.cleanDescriptions)} <span>decisões</span></div><p>substituem a revisão manual das {formatCount(summaryData.marketLines)} linhas porque cada descrição limpa é tratada uma única vez.</p><div className="coverage-meter"><div style={{ width: `${Math.round((1 - summaryData.cleanDescriptions / summaryData.marketLines) * 100)}%` }} /></div><small>Repetições preservadas para preços, marcas e demanda</small></article>
      ...
      {view === "validation" && <div className="page mapping-page">
        <section className="section-intro compact"><div><span className="eyebrow">PADRONIZAÇÃO ASSISTIDA</span><h2>Descrições prontas para validar</h2><p>A tabela compara o menor preço do mercado com o preço Sogamax e apresenta o CMV correspondente a cada valor. Clique em qualquer produto para abrir todos os detalhes.</p></div></section>
        <div className="mapping-layout validation-full">
          <section className="panel table-panel"><div className="table-toolbar"><label className="search-box"><span>⌕</span><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar descrição recebida ou descrição Sogamax…" /></label><select value={filter} onChange={(event) => setFilter(event.target.value as "Todos" | MatchStatus)} aria-label="Filtrar por situação"><option>Todos</option><option>Padronizado</option><option>Provável</option><option>Revisar</option></select></div><div className="table-wrap"><table className="validation-table commercial-table"><thead><tr><th>Descrição recebida → padrão Sogamax</th><th>Menor preço</th><th>CMV</th><th>Concorrente / marca</th><th>Preço Sogamax</th><th>Último custo comprado</th><th>Custo médio</th><th>CMV</th><th>Sugestão de compra</th><th>CMV</th></tr></thead><tbody>{filteredRows.map((row) => { const info = priceSummary(row.id); const mapped = row.status !== "Revisar"; const marketCmv = mapped && info.data.sogamaxCost > 0 ? info.data.sogamaxCost / info.lowest.price * 100 : null; const sogamaxCmv = mapped && info.data.sogamaxPrice > 0 ? info.data.sogamaxCost / info.data.sogamaxPrice * 100 : null; const purchaseSuggestion = purchaseSuggestions[row.id] ?? ""; const purchaseSuggestionValue = Number(purchaseSuggestion.replace(",", ".")); const suggestionCmv = mapped && purchaseSuggestion.trim() !== "" && purchaseSuggestionValue > 0 && Number.isFinite(purchaseSuggestionValue) ? info.data.sogamaxCost / purchaseSuggestionValue * 100 : null; return <tr key={row.id} className={selected.id === row.id ? "selected" : ""} onClick={() => openProduct(row.id)}><td><strong>{row.marketDescription}</strong><small>{row.standardDescription}</small></td><td><strong>{money(info.lowest.price)}</strong></td><td><strong className="table-cmv">{marketCmv === null ? "—" : `${marketCmv.toFixed(2).replace(".", ",")}%`}</strong></td><td><strong>{info.lowest.competitor}</strong><small>{info.lowest.brand}</small></td><td><strong>{mapped ? money(info.data.sogamaxPrice) : "—"}</strong><small>{mapped ? "tabela enviada" : "aguardando validação"}</small></td><td><strong>{mapped ? money(info.data.lastPurchaseCost) : "—"}</strong></td><td><strong>{mapped ? money(info.data.sogamaxCost) : "—"}</strong></td><td><strong className="table-cmv">{sogamaxCmv === null ? "—" : `${sogamaxCmv.toFixed(2).replace(".", ",")}%`}</strong></td><td><label className="purchase-input" onClick={(event) => event.stopPropagation()}><span>R$</span><input value={purchaseSuggestion} inputMode="decimal" aria-label={`Sugestão de compra para ${row.standardDescription}`} onChange={(event) => setPurchaseSuggestions((current) => ({ ...current, [row.id]: event.target.value }))} onKeyDown={(event) => event.stopPropagation()} /></label></td><td><strong className="table-cmv">{suggestionCmv === null ? "—" : `${suggestionCmv.toFixed(2).replace(".", ",")}%`}</strong></td></tr>})}</tbody></table></div></section>
        </div>
      </div>}

      {view === "export" && <div className="page"><section className="section-intro"><div><span className="eyebrow">SAÍDA DA ANÁLISE</span><h2>Relatório com descrições padronizadas</h2><p>As {formatCount(summaryData.marketLines)} linhas permanecem no relatório. Cada repetição recebe a mesma descrição Sogamax aprovada, preservando preços, demandas, marcas e concorrentes.</p></div></section><section className="export-grid"><article className="panel export-card"><div className="file-badge">CSV</div><div><span className="eyebrow">ARQUIVO FINAL</span><h3>Mercado hospitalar padronizado</h3><p>Inclui descrição original, descrição padrão Sogamax, repetições, confiança, menor preço, concorrente, marca, preço Sogamax, demanda padronizada e situação da validação.</p></div><div className="export-stats"><div><strong>{formatCount(summaryData.marketLines)}</strong><span>linhas preservadas</span></div><div><strong>{formatCount(summaryData.cleanDescriptions)}</strong><span>descrições limpas</span></div><div><strong>{approved.length}</strong><span>padrões aprovados</span></div></div><button className="primary wide" onClick={() => downloadCsv(approved)}>Baixar demonstração em CSV <span>↓</span></button></article><aside className="panel release-card"><span className="eyebrow">CONTEÚDO DO ARQUIVO</span><h3>Pronto para análise comercial</h3>{["Descrição única da Sogamax", "Menor preço e responsável", "Preço praticado pela Sogamax", "Demanda em unidade-base", "Marcas e concorrentes"].map((item) => <div className="check-item ok" key={item}><span>✓</span><div><strong>{item}</strong></div></div>)}</aside></section></div>}
    </section>
...
function ProductPopup({ kind, row, data, onChange, onClose }: { kind: PopupKind; row: DescriptionRow; data: ProductData; onChange: (kind: PopupKind) => void; onClose: () => void }) {
  ...
  const brands = [...new Set(offers.map((offer) => offer.brand))].map((brand) => { const group = offers.filter((offer) => offer.brand === brand); return { brand, competitor: group.map((offer) => offer.competitor).join(" · "), demand: group.reduce((sum, offer) => sum + offer.quantity, 0) }; }).sort((a, b) => b.demand - a.demand);
  ...
  return <div className="modal-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><section className="market-modal" role="dialog" aria-modal="true"><header className="modal-header"><div><span className="eyebrow">DETALHES DO PRODUTO</span><h2>{row.standardDescription}</h2><p>Recebido como: {row.marketDescription} · aparece {row.repetitions} vezes</p></div><button className="modal-close" onClick={onClose} aria-label="Fechar">×</button></header><div className="modal-tabs"><button className={kind === "prices" ? "active" : ""} onClick={() => onChange("prices")}>Preços</button><button className={kind === "demands" ? "active" : ""} onClick={() => onChange("demands")}>Demanda padronizada</button><button className={kind === "brands" ? "active" : ""} onClick={() => onChange("brands")}>Marcas</button></div>
    {kind === "prices" && <div className="modal-content"><div className="popup-table-wrap"><table className="popup-table"><thead><tr><th>Posição</th><th>Empresa</th><th>Marca</th><th>Apresentação</th><th>VL Venda</th><th>Custo</th><th>CMV Sogamax</th></tr></thead><tbody>{rankedPrices.map((offer, index) => { const cmv = offer.price > 0 ? data.sogamaxCost / offer.price * 100 : null; const cost = cmv === null ? 0 : offer.price * (cmv / 100); return <tr key={`${offer.competitor}-${offer.price}`} className={offer.competitor === "SOGAMAX" ? "sogamax-row" : ""}><td><span className="price-rank">{index + 1}º</span></td><td><strong>{offer.competitor}</strong></td><td>{offer.brand}</td><td>{offer.unit}{offer.packSize > 1 ? ` C/${offer.packSize}` : ""}</td><td><strong>{money(offer.price)}</strong></td><td><strong>{money(cost)}</strong></td><td><strong className="cmv-value">{cmv === null ? "—" : `${cmv.toFixed(2).replace(".", ",")}%`}</strong></td></tr> })}</tbody></table></div><div className="modal-note">Preços ordenados automaticamente do menor para o maior, incluindo a Sogamax na posição correspondente. Valores carregados das três bases enviadas em 14/09/2026.</div></div>}
    {kind === "demands" && <div className="modal-content"><div className="modal-kpis"><div><span>Demanda convertida</span><strong>{totalDemand.toLocaleString("pt-BR")} {offers[0]?.baseUnit}</strong></div><div><span>Registros convertidos</span><strong>{converted.length}</strong></div><div><span>Aguardando regra</span><strong>{pending.length}</strong></div><div><span>Unidade-base</span><strong>{offers[0]?.baseUnit}</strong></div></div><div className="popup-table-wrap"><table className="popup-table"><thead><tr><th>Concorrente</th><th>Demanda original</th><th>Regra aplicada</th><th>Demanda padronizada</th><th>Situação</th></tr></thead><tbody>{offers.map((offer) => <tr key={`${offer.competitor}-${offer.quantity}`}><td><strong>{offer.competitor}</strong></td><td>{offer.quantity.toLocaleString("pt-BR")} {offer.unit}</td><td>Quantidade já informada em {offer.baseUnit}</td><td><strong>{offer.quantity > 0 ? `${offer.quantity.toLocaleString("pt-BR")} ${offer.baseUnit}` : "—"}</strong></td><td>{offer.quantity > 0 ? <span className="conversion ok">Convertido</span> : <span className="conversion pending">Validar conversão</span>}</td></tr>)}</tbody></table></div>{pending.length > 0 && <div className="conversion-alert"><strong>{pending.length} registro fora do total.</strong><span>Registros sem quantidade não são somados.</span></div>}</div>}
    {kind === "brands" && <div className="modal-content"><div className="brand-summary"><div><span className="eyebrow">MARCAS ENCONTRADAS</span><strong>{brands.length}</strong><p>Visão consolidada das marcas oferecidas para esta descrição padronizada.</p></div><div className="brand-ring" style={{ "--brand-count": brands.length } as React.CSSProperties}><span>{brands.length}</span><small>marcas</small></div></div><div className="brand-grid">{brands.map((brand, index) => <article key={brand.brand}><div className="brand-rank">0{index + 1}</div><div><h3>{brand.brand}</h3><p>{brand.competitor}</p></div><dl className="brand-demand-only"><div><dt>Demanda convertida</dt><dd>{brand.demand.toLocaleString("pt-BR")} {offers[0]?.baseUnit}</dd></div></dl></article>)}</div></div>}
  </section></div>;
}
