...
function ProductPopup({ kind, row, data, onChange, onClose }: { kind: PopupKind; row: DescriptionRow; data: ProductData; onChange: (kind: PopupKind) => void; onClose: () => void }) {
  ...
  const cmvFor = (price: number) => `${(data.sogamaxCost / price * 100).toFixed(2).replace(".", ",")}%`;
  ...
  return <div className="modal-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><section className="market-modal" role="dialog" aria-modal="true"><header className="modal-header"><div><span className="eyebrow">DETALHES DO PRODUTO</span><h2>{row.standardDescription}</h2><p>Recebido como: {row.marketDescription} · aparece {row.repetitions} vezes</p></div><button className="modal-close" onClick={onClose} aria-label="Fechar">×</button></header><div className="modal-tabs"><button className={kind === "prices" ? "active" : ""} onClick={() => onChange("prices")}>Preços</button><button className={kind === "demands" ? "active" : ""} onClick={() => onChange("demands")}>Demanda padronizada</button><button className={kind === "brands" ? "active" : ""} onClick={() => onChange("brands")}>Marcas</button></div>
    {kind === "prices" && <div className="modal-content"><div className="modal-kpis price-kpis"><div className="sogamax-kpi"><span>Preço praticado Sogamax</span><div className="price-with-cmv"><strong>{money(data.sogamaxPrice)}</strong><b>CMV {cmvFor(data.sogamaxPrice)}</b></div><small>demonstrativo</small></div><div><span>Menor preço do mercado</span><div className="price-with-cmv"><strong>{money(minimum.price)}</strong><b>CMV {cmvFor(minimum.price)}</b></div><small>{minimum.competitor} · {minimum.brand}</small></div><div><span>Preço médio do mercado</span><div className="price-with-cmv"><strong>{money(average)}</strong><b>CMV {cmvFor(average)}</b></div><small>média dos preços concorrentes</small></div><div><span>Maior preço do mercado</span><div className="price-with-cmv"><strong>{money(maximum.price)}</strong><b>CMV {cmvFor(maximum.price)}</b></div><small>{maximum.competitor} · {maximum.brand}</small></div></div><div className="winner-banner"><span>MENOR PREÇO GERAL</span><strong>{absoluteMinimum === data.sogamaxPrice ? "SOGAMAX" : minimum.competitor}</strong><small>{absoluteMinimum === data.sogamaxPrice ? "Preço praticado Sogamax" : minimum.brand} · {money(absoluteMinimum)}</small></div><div className="popup-table-wrap"><table className="popup-table"><thead><tr><th>Posição</th><th>Empresa</th><th>Marca</th><th>Apresentação</th><th>Preço de venda</th><th>Nosso CMV</th></tr></thead><tbody>{rankedPrices.map((offer, index) => { const cmv = data.sogamaxCost / offer.price * 100; return <tr key={`${offer.competitor}-${offer.price}`} className={offer.competitor === "SOGAMAX" ? "sogamax-row" : ""}><td><span className="price-rank">{index + 1}º</span></td><td><strong>{offer.competitor}</strong></td><td>{offer.brand}</td><td>{offer.unit}{offer.packSize > 1 ? ` C/${offer.packSize}` : ""}</td><td><strong>{money(offer.price)}</strong></td><td><strong className="cmv-value">{cmv.toFixed(2).replace(".", ",")}%</strong><small className="cmv-source">custo ÷ preço de venda</small></td></tr> })}</tbody></table></div><div className="modal-note">Preços ordenados automaticamente do menor para o maior, incluindo a Sogamax na posição correspondente. CMV = custo atual Sogamax ÷ preço de venda × 100. Os dados permanecem demonstrativos no protótipo; na versão integrada, o custo atual virá do ERP.</div></div>}
