...
function ProductPopup({ kind, row, data, onChange, onClose }: { kind: PopupKind; row: DescriptionRow; data: ProductData; onChange: (kind: PopupKind) => void; onClose: () => void }) {
  ...
  const brands = [...new Set(offers.map((offer) => offer.brand))].map((brand) => { const group = offers.filter((offer) => offer.brand === brand); return { brand, competitor: group.map((offer) => offer.competitor).join(" · "), demand: group.reduce((sum, offer) => sum + (offer.packSize === null ? 0 : offer.quantity * offer.packSize), 0) }; });
