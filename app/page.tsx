...
const productData: Record<number, ProductData> = {
  1: { sogamaxPrice: 8.90, sogamaxCost: 6.34, lastPurchaseCost: 6.41, offers: [
  ...
  2: { sogamaxPrice: 18.70, sogamaxCost: 13.82, lastPurchaseCost: 13.96, offers: [
  ...
  3: { sogamaxPrice: 14.55, sogamaxCost: 10.41, lastPurchaseCost: 10.35, offers: [
  ...
  4: { sogamaxPrice: 16.20, sogamaxCost: 11.76, lastPurchaseCost: 11.88, offers: [
  ...
  5: { sogamaxPrice: 0.18, sogamaxCost: 0.11, lastPurchaseCost: 0.12, offers: [
  ...
  6: { sogamaxPrice: 0.74, sogamaxCost: 0.48, lastPurchaseCost: 0.49, offers: [
  ...
  7: { sogamaxPrice: 0.12, sogamaxCost: 0.08, lastPurchaseCost: 0.08, offers: [
  ...
  8: { sogamaxPrice: 3.36, sogamaxCost: 2.29, lastPurchaseCost: 2.34, offers: [
...
function ProductPopup({ kind, row, data, onChange, onClose }: { kind: PopupKind; row: DescriptionRow; data: ProductData; onChange: (kind: PopupKind) => void; onClose: () => void }) {
  ...
  return <div className="modal-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><section className="market-modal" role="dialog" aria-modal="true"><header className="modal-header"><div><span className="eyebrow">DETALHES DO PRODUTO</span><h2>{row.standardDescription}</h2><p>Recebido como: {row.marketDescription} · aparece {row.repetitions} vezes</p></div><button className="modal-close" onClick={onClose} aria-label="Fechar">×</button></header><div className="modal-tabs"><button className={kind === "prices" ? "active" : ""} onClick={() => onChange("prices")}>Preços</button><button className={kind === "demands" ? "active" : ""} onClick={() => onChange("demands")}>Demanda padronizada</button><button className={kind === "brands" ? "active" : ""} onClick={() => onChange("brands")}>Marcas</button></div>
    {kind === "prices" && <div className="modal-content"><div className="popup-table-wrap"><table className="popup-table"><thead><tr><th>Posição</th><th>Empresa</th><th>Marca</th><th>Apresentação</th><th>VL Venda</th><th>Custo</th><th>CMV Sogamax</th></tr></thead><tbody>{rankedPrices.map((offer, index) => { const cmv = data.sogamaxCost / offer.price * 100; const cost = offer.price * (cmv / 100); return <tr key={`${offer.competitor}-${offer.price}`} className={offer.competitor === "SOGAMAX" ? "sogamax-row" : ""}><td><span className="price-rank">{index + 1}º</span></td><td><strong>{offer.competitor}</strong></td><td>{offer.brand}</td><td>{offer.unit}{offer.packSize > 1 ? ` C/${offer.packSize}` : ""}</td><td><strong>{money(offer.price)}</strong></td><td><strong>{money(cost)}</strong></td><td><strong className="cmv-value">{cmv.toFixed(2).replace(".", ",")}%</strong></td></tr> })}</tbody></table></div><div className="modal-note">Preços ordenados automaticamente do menor para o maior, incluindo a Sogamax na posição correspondente. Os dados permanecem demonstrativos no protótipo; na versão integrada, os custos virão do ERP.</div></div>}
